...
def test_confirmable_flag(base_app, client, sqlalchemy_datastore, get_message):
    recorded_confirms = []
    recorded_instructions_sent = []

    @user_confirmed.connect_via(base_app)
    def on_confirmed(base_app, user):
        assert isinstance(base_app, Flask)
        assert isinstance(user, UserMixin)
        recorded_confirms.append(user)

    @confirm_instructions_sent.connect_via(base_app)
    ...
    base_app.config["SECURITY_CONFIRM_EMAIL_WITHIN"] = "-1 days"
...
def test_email_conflict_for_confirmation_token(
    base_app, client, get_message, sqlalchemy_datastore
):
    with capture_registrations() as registrations:
        data = dict(email="mary@lp.com", password="password", next="")
        client.post("/register", data=data, follow_redirects=True)
