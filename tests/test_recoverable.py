...
def test_recoverable_flag(base_app, client, get_message):
    recorded_resets = []
    recorded_instructions_sent = []

    @password_reset.connect_via(base_app)
    def on_password_reset(base_app, user):
        recorded_resets.append(user)

    @reset_password_instructions_sent.connect_via(base_app)
    def on_instructions_sent(base_app, user, token):
...
def test_login_form_description(sqlalchemy_app):
    ...
    with base_app.test_request_context("/login"):
...
def test_reset_token_deleted_user(base_app, client, get_message, sqlalchemy_datastore):
    with capture_reset_password_requests() as requests:
