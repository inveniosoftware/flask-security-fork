...
def test_registerable_flag(client, base_app, get_message):
    recorded = []
    ...
    @user_registered.connect_via(base_app)
    def on_user_registerd(base_app, user, confirm_token):
...
def test_disable_register_emails(client, base_app):
    data = dict(
        email="dude@lp.com", password="password", password_confirm="password", next=""
    )
    with base_app.mail.record_messages() as outbox:
